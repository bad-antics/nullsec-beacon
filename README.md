<!-- 
SEO Keywords: NullSec Beacon, OCaml network beacon, C2 beacon, callback, heartbeat,
covert communication, command and control, red team tools, persistence, implant,
bad-antics, NullSec Framework, post-exploitation, beacon framework
-->

<div align="center">

# 📡 NullSec Beacon

### Lightweight Network Beacon & Callback System

[![Discord](https://img.shields.io/badge/🔑_GET_KEYS-discord.gg/killers-5865F2?style=for-the-badge&logo=discord&logoColor=white)](https://discord.gg/killers)
[![GitHub](https://img.shields.io/badge/GitHub-bad--antics-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/bad-antics)
[![License](https://img.shields.io/badge/License-NBN--XXX-red?style=for-the-badge)](LICENSE)

[![OCaml](https://img.shields.io/badge/OCaml-EC6813?style=for-the-badge&logo=ocaml&logoColor=white)]()

```
    ███▄    █  █    ██  ██▓     ██▓      ██████ ▓█████  ▄████▄  
    ██ ▀█   █  ██  ▓██▒▓██▒    ▓██▒    ▒██    ▒ ▓█   ▀ ▒██▀ ▀█  
   ▓██  ▀█ ██▒▓██  ▒██░▒██░    ▒██░    ░ ▓██▄   ▒███   ▒▓█    ▄ 
   ▓██▒  ▐▌██▒▓▓█  ░██░▒██░    ▒██░      ▒   ██▒▒▓█  ▄ ▒▓▓▄ ▄██▒
   ▒██░   ▓██░▒▒█████▓ ░██████▒░██████▒▒██████▒▒░▒████▒▒ ▓███▀ ░
   ░ ▒░   ▒ ▒ ░▒▓▒ ▒ ▒ ░ ▒░▓  ░░ ▒░▓  ░▒ ▒▓▒ ▒ ░░░ ▒░ ░░ ░▒ ▒  ░
   ▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
   █░░░░░░░░░░░░░░░░░ B E A C O N ░░░░░░░░░░░░░░░░░░░░░░░░░░░░█
   ▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀
                       bad-antics
```

### 🔓 **[Join discord.gg/killers](https://discord.gg/killers)** for premium features!

</div>

---

## 🎯 Features

- 📡 **HTTP/HTTPS Beacons** - Web-based callbacks with jitter
- 🌐 **DNS Beacons** - Covert DNS-based communication
- 🔄 **ICMP Beacons** - Ping-based heartbeat
- 🔐 **Encrypted Comms** - AES-256 encrypted payloads
- ⏰ **Configurable Jitter** - Randomized callback intervals
- 🎭 **Profile Malleable** - Customizable traffic profiles

---

## 🚀 Usage

```bash
# Server mode (listener)
./beacon server -p 8443 --ssl

# Client mode (beacon)
./beacon client -s 192.168.1.100 -p 8443 -i 30 -j 20

# DNS beacon mode
./beacon dns-client -d beacon.example.com -s ns.example.com
```

---

<div align="center">

**[Discord](https://discord.gg/killers)** • **[GitHub](https://github.com/bad-antics)**

*Made with 💀 by bad-antics*

</div>
